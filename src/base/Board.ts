...
export interface BoardRenderer {
    type: 'no';
    displayCopyright(text: string, size: number): void;
}
...
export class Board {
    readonly BOARD_MODE_NONE: number;
    readonly BOARD_MODE_DRAG: number;
    readonly BOARD_MODE_MOVE_ORIGIN: number;
    readonly BOARD_MODE_ZOOM: number;
    readonly BOARD_QUALITY_LOW: number;
    readonly BOARD_QUALITY_HIGH: number;
    private readonly document: Document;
    private readonly container: string;
    private readonly containerObj: HTMLElement;
    private readonly renderer: BoardRenderer;
    private readonly grids: unknown[];
    private readonly options: unknown;
    private readonly attr: BoardAttributes;
    private readonly dimension: 2;
    private readonly jc: JessieCode;
    private readonly origin: Origin;
    private readonly zoomX: number;
    private readonly zoomY: number;
    ...
    private readonly dependentBoards: Board[];
    private readonly keepaspectratio: boolean;
    private inUpdate: boolean;
    private readonly objects: { [id: string]: unknown };
    private readonly objectsList: unknown[];
    private readonly groups: { [id: string]: unknown };
    private readonly animationObjects: { [something: string]: unknown };
    private readonly highlightedObjects: { [something: string]: unknown };
    private readonly numObjects: number;
    readonly elementsByName: { [name: string]: unknown };
    private readonly isSuspendedRedraw: boolean;
    private mode: number;
    private updateQuality: number;
    private drag_dx: number;
    private drag_dy: number;
    private drag_position: [number, number];
    private mouse: unknown;
    private touches: unknown[];
    private readonly xmlString: string;
    private readonly cPos: unknown[];
    private readonly touchMoveLast: number;
    private readonly touchMoveLastId: number;
    private positionAccessLast: number;
    private readonly downObjects: unknown[];
    private readonly needsFullUpdate: boolean;
    private readonly reducedUpdate: boolean;
    private readonly currentCBDef: 'none';
    private readonly geonextCompatibilityMode: boolean;
    private readonly hasMouseHandlers: boolean;
    private hasTouchHandlers: boolean;
    private hasPointerHandlers: boolean;
    private hasMouseUp: boolean;
    private hasTouchEnd: boolean;
    private hasPointerUp: boolean;
    private readonly _drag_offset: [number, number];
    private _inputDevice: 'mouse' | 'touch' | 'pen';
    private readonly _board_touches: unknown[];
    private readonly selectingMode: boolean;
    private readonly isSelecting: boolean;
    private readonly _isScrolling: boolean;
    private readonly _isResizing: boolean;
    private readonly selectingBox: [[number, number], [number, number]];
    private readonly mathLib: unknown;
    private readonly mathLibJXG: unknown;
    private readonly methodMap: { [name: string]: string };
    /**
     ...
     */
    constructor(container: string, renderer: BoardRenderer, id: string, origin: Coords, zoomX: number, zoomY: number, unitX: number, unitY: number, canvasWidth: number, canvasHeight: number, attributes: BoardAttributes) {
        /**
        ...
        if (exists(attributes.document) && attributes.document !== false) {
            this.document = attributes.document;
        } else if (document !== undefined && isObject(document)) {
         ...
         * @type Object
         */
        this.containerObj = (isBrowser ? this.document.getElementById(this.container) : null);

        if (isBrowser && renderer.type !== 'no' && this.containerObj === null) {
        ...
        if (exists(id) && id !== '' && isBrowser && !exists(this.document.getElementById(id))) {
            this.id = id;
        } else {
    ...
    generateName(object) {
        ...
        while (indices[maxNameLength - 1] < possibleNames.length) {
            for (indices[0] = 1; indices[0] < possibleNames.length; indices[0]++) {
                ...
                if (!exists(this.elementsByName[name + post])) {
                    return name + post;
                }
    ...
    generateId() {
        ...
        while (exists(JXG.boards['jxgBoard' + r])) {
    ...
    setId(obj, type) {
        ...
        if (elId === '' || !exists(elId)) {
            ...
            while (exists(this.objects[elId])) {
    ...
    checkFrameRate(evt) {
        ...
        if (exists(evt.pointerId) && this.touchMoveLastId !== evt.pointerId) {
            handleEvt = true;
    ...
    getCoordsTopLeftCorner() {
        ...
        if (container.getBoundingClientRect) {
            ...
            o = container;
            while (o && exists(o.parentNode)) {
                if (exists(o.style) && exists(o.style.zoom) && o.style.zoom !== '') {
     ...
     */
    initMoveOrigin(x: number, y: number): void {
            ...
            if (haspoint &&
                ...
                if (pEl.visProp.layer > dragEl.visProp.layer ||
                    ...
                    if (!this.attr.ignorelabels ||
                        (!exists(dragEl.label) || pEl !== dragEl.label)) {
                        dragEl = pEl;
                        collect.push(dragEl);

                        // Save offset for large coords elements.
                        if (exists(dragEl.coords)) {
    ...
    highlightElements(x, y, evt, target) {
        ...
        for (el = 0; el < len; el++) {
            pEl = this.objectsList[el];
            pId = pEl.id;
            if (exists(pEl.hasPoint) && pEl.visPropCalc.visible && pEl.hasPoint(x, y)) {
                // this is required in any case because otherwise the box won't be shown until the point is dragged
                this.updateInfobox(pEl);

                if (!exists(this.highlightedObjects[pId])) { // highlight only if not highlighted
                    overObjects[pId] = pEl;
                    pEl.highlight();
    ...
    addTouchEventHandlers(appleGestures) {
        if (!this.hasTouchHandlers && Env.isBrowser) {
            ...
            /*
            if (!exists(appleGestures) || appleGestures) {
    ...
    keyDownListener(evt) {
        ...
        if ((Type.evaluate(this.attr.keyboard.panshift) && evt.shiftKey) ||
            ...
            if (exists(el.visProp)) {
                if (exists(el.visProp.snaptogrid) &&
                ...
                } else if (exists(el.visProp.attracttogrid) &&
    ...
    stopIntersectionObserver() {
        if (exists(this.intersectionObserver)) {
    ...
    */
    ...
    dehighlightAll() {
        ...
        for (el in this.highlightedObjects) {
            if (this.highlightedObjects.hasOwnProperty(el)) {
                ...
                // if ((!exists(pEl.hasPoint)) || !pEl.hasPoint(x, y) || !pEl.visPropCalc.visible)
            }
        }
     ...
     * @returns {JXG.Board} Reference to this board.
     */
    moveOrigin(x: number, y: number, diff = false): this {
        var ox, oy, ul, lr;
    ...
    addConditions(str) {
        ...
        while (i >= 0) {
            ...
            // Debug
            if (!exists(this.elementsByName[name])) {
    ...
    zoom100() {
        ...
        if (exists(this.attr.boundingbox)) {
    ...
    removeObject(object, saveMethod) {
        ...
        if (!exists(object) || Type.isString(object)) {
            return this;
        }
        ...
        try {
            ...
            if (saveMethod) {
                ...
                for (el in this.objects) {
                    if (this.objects.hasOwnProperty(el)) {
                        if (exists(this.objects[el].childElements) &&
                            exists(this.objects[el].childElements.hasOwnProperty(object.id))
                        ) {
            ...
            } else if (exists(object.ancestors)) {
                ...
                for (el in object.ancestors) {
                    if (object.ancestors.hasOwnProperty(el)) {
                        if (exists(object.ancestors[el].childElements) &&
                            exists(object.ancestors[el].childElements.hasOwnProperty(object.id))
                        ) {
    ...
    updateElements(drag) {
        ...
        */
        ...
        for (el = 0; el < this.objectsList.length; el++) {
            ...
            pEl.update(!exists(drag) || pEl.id !== drag.id)
                .updateVisibility();
        }
     ...
     */
    addChild(board) {
        if (exists(board) && exists(board.containerObj)) {
    ...
    update(drag?: unknown) {
        ...
        const len = this.dependentBoards.length;
        for (let i = 0; i < len; i++) {
            const b = this.dependentBoards[i];
            if (exists(b) && b !== this) {
     ...
     */
    create(elementType: string, parents: unknown[], attributes) {
        var el;
    ...
    setBoundingBox(bbox, keepaspectratio, setZoom) {
        ...
        if (setZoom === 'update') {
            ...
            this.zoomX = exists(this.attr.zoomx) ? this.attr.zoomx : 1.0;
            this.zoomY = exists(this.attr.zoomy) ? this.attr.zoomy : 1.0;
        }
    ...
    stopAllAnimation() {
        ...
        for (el in this.animationObjects) {
            if (this.animationObjects.hasOwnProperty(el) && exists(this.animationObjects[el])) {
        ...
        for (el in this.animationObjects) {
            if (this.animationObjects.hasOwnProperty(el) && exists(this.animationObjects[el])) {
                ...
                if (o.animationPath) {
                    ...
                    if ((!exists(newCoords)) || (!Type.isArray(newCoords) && isNaN(newCoords))) {
                ...
                if (!exists(o.animationData) && !exists(o.animationPath)) {
                    this.animationObjects[el] = null;
                    delete this.animationObjects[el];

                    if (exists(o.animationCallback)) {
    ...
    emulateColorblindness(deficiency) {
        ...
        for (e in this.objects) {
            if (this.objects.hasOwnProperty(e)) {
                ...
                } else if (exists(o.visPropOriginal)) {
    ...
    select(str, onlyByIdOrName) {
        ...
        if (Type.isString(s) && s !== '') {
            // Search by ID
            if (exists(this.objects[s])) {
                s = this.objects[s];
                // Search by name
            } else if (exists(this.elementsByName[s])) {
                s = this.elementsByName[s];
                // Search by group ID
            } else if (exists(this.groups[s])) {
                s = this.groups[s];
            }
        ...
        } else if (Type.isObject(s) && exists(s.id) && !exists(this.objects[s.id])) {
            s = null;
        }
    ...
    hasPoint(x, y) {
        ...
        if (exists(x) && Type.isArray(x.usrCoords)) {
            px = x.usrCoords[1];
            py = x.usrCoords[2];
        }
    ...
    _testForSelection(evt) {
        if (this._isRequiredKeyPressed(evt, 'selection')) {
            if (!exists(this.selectionPolygon)) {
    ...
    */
    ...
    fullscreenListener(evt) {
        ...
        } else if (exists(inner_node._cssFullscreenStore)) {
